...
class Picture(object): 
    ...
    def __init__(self, theImagePath, SZ, MIN_SIGMA, MAX_SIGMA, SIGMA_FACTOR):
        ...
        self.fatFiducialDataPlane = np.zeros((self.embeddingData['hh'], self.embeddingData['ww']), dtype=np.uint8)
        self.EmbedImage()
    ...
    def resizedOriginalImage(self):
        ...
        return (Image.open(self.theImagePath)).resize((width, height), resample = Image.LANCZOS)
    ...
    def EmbedImage(self):
        ...
        fiducialDataPlane = np.array(self.fiducialImage, dtype=np.uint8)
        ...
        self.fatFiducialDataPlane[dh:h+dh, 0:dw] = np.fliplr(fiducialDataPlane[:, 0:dw])
        # copies original picture in the middle of fatFiducialDataPlane
        self.fatFiducialDataPlane[dh:h+dh, dw:dw+w] = fiducialDataPlane
        # copies flipped (lr) right part (dw->w) of original picture to right and middle of fatFiducialDataPlane
        self.fatFiducialDataPlane[dh:h+dh, ww-dw:ww] = np.fliplr(fiducialDataPlane[:, w-dw:w])
        ...
        self.fatFiducialDataPlane[0:dh, :] = np.flipud(self.fatFiducialDataPlane[dh:2*dh, :])
        ...
        self.fatFiducialDataPlane[hh-dh:hh, :] = np.flipud(self.fatFiducialDataPlane[hh-2*dh:hh-dh, :])

    def DisembedDataPlane(self, dataPlane):
        w  = self.embeddingData['w']
        h  = self.embeddingData['h']
        dw = self.embeddingData['dw']
        dh = self.embeddingData['dh']
        return (dataPlane[dh:h+dh, dw:w+dw]).astype('uint8')
