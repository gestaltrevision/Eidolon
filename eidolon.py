...
#    pic = Picture('Rudy_Dekeerschieter_ZW.jpg', SZ, MIN_SIGMA, MAX_SIGMA, SIGMA_FACTOR)
    ...
    # What are we running?
    #==============================================================================
    if   PRECOOKED_EXAMPLE == SUPERFICIAL_DISARRAY: SuperficialDisarray(pic,REACH,GRAIN)
    elif PRECOOKED_EXAMPLE == LOTZE_DISARRAY: LotzeTypeDisarray(pic,REACH, GRAIN) 
    elif PRECOOKED_EXAMPLE == HELMHOLTZ_DISARRAY: HelmholtzTypeDisarray(REACH) 
    elif PRECOOKED_EXAMPLE == COHERENT_DISARRAY: CoherentDisarrayOfEdges(REACH) 
    elif PRECOOKED_EXAMPLE == ORIENTED_EDGE_DISARRAY: OrientedEdgeDisarray(REACH) 
    elif PRECOOKED_EXAMPLE == DISPLAY_SCALESPACE_LEVEL: DisplayScalespaceLevel(LEVEL) 
    elif PRECOOKED_EXAMPLE == DISPLAY_FRACTAL_NOISE_PLANE: DisplayFractalNoise() 
    elif PRECOOKED_EXAMPLE == CHROMATIC_ABERRATION: CoherentDisarrayOfEdgesRGBChannels(REACH) 
    elif PRECOOKED_EXAMPLE == EDGE_DIRECTION_MAP: DisplayGradientDirection(LEVEL) 
    elif PRECOOKED_EXAMPLE == SANITY_CHECK_I: SynthesisFromDOGActivity() 
    elif PRECOOKED_EXAMPLE == SANITY_CHECK_III: SynthesisFromSimpleCellActivity() 
    elif PRECOOKED_EXAMPLE == SANITY_CHECK_II: SynthesisFromLaplacian() 
    elif PRECOOKED_EXAMPLE == LARGEST_DOG_ACTIVITY: SynthesisFromLargestDOGActivity(FRACTION) 
    elif PRECOOKED_EXAMPLE == GRADIENT_FIELD: GradientField(LEVEL) 
    elif PRECOOKED_EXAMPLE == LINE_FINDER_FIELD: LineFinderField(LEVEL) 
    elif PRECOOKED_EXAMPLE == PARTIAL_COHERENCE: PartiallyCoherentDisarray(REACH,INCOHERENCE,GRAIN) 
    elif PRECOOKED_EXAMPLE == SUPERFICIAL_OPPONENT: CoherentDisarrayOfEdgesOpponentChannels(REACH,GRAIN) 
    elif PRECOOKED_EXAMPLE == LOTZE_OPPONENT: LotzeDisarrayOfEdgesOpponentChannels(REACH,GRAIN) 
    elif PRECOOKED_EXAMPLE == COHERENT_OPPONENT: CoherentDisarrayOfOpponentChannels(REACH) 
    elif PRECOOKED_EXAMPLE == HELMHOLTZ_OPPONENT: HelmholtzDisarrayOfEdgesOpponentChannels(REACH)
    elif PRECOOKED_EXAMPLE == MISSING_FROM_REPRESENTATION: MissingFromScaleSpaceRepresentation() 
    elif PRECOOKED_EXAMPLE == DISCRETIZATION_ERROR: DiscretizationError() 
    else: pass
