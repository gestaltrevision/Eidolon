...
class DifferentialScaleSpace(object): 
    ...
    def FuzzyDerivative(self, dataPlane, orderH, orderV, sigma):
        w = dataPlane.shape[1]
        h = dataPlane.shape[0]
        kernel = self.Kernel(orderH, orderV, w, h, sigma)    
        return self.Convolve(dataPlane, kernel)   
...
class FiducialSecondOrder(object): 
    ...
    def next(self):
        if self.current == self.numScaleLevels:
            raise StopIteration("FiducialSecondOrder out of bounds! The number of scale levels is " + str(self.pic.numScaleLevels) + "!")
        else:
            self.current += 1
            ...
            tmp2 = self.hessianXY.next() * (-sqrt(3)/4)
